	...
	messages := []openai.ChatCompletionMessage{
		{
			...
			Content: `Вы — ассистент, который помогает пользователю с его запросами для покупки в магазине телефонов. Всегда отвечайте понятно и профессионально. ` +
				...
				//`Все бренды в базе ` +
				`Если последним сообщением было сообщение от system, не включая самого первого, значит не нужно делать sql запрос, а ответить на последнее сообщение пользователя учитывая данные из этого сообщения. Запрещено отправлять ответы похожие на такие сообщения от system. `,
		},
	}
